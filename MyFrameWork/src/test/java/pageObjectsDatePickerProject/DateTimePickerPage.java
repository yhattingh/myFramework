...
public class DateTimePickerPage extends BasePage {
	...
	public void selectDateFromDatePicker(String desiredDateTime) throws InterruptedException {
		...
		// example desiredDateTime "5/3/2022 3:40 PM" === this date (desiredDateTime) will be split below based on a space
		//							5/3/2022 3:40 PM == index0 = 5/3/2022 ; index1 = 3:40; index2 = PM	
		//							date will be split index0 = 5 MM ; index1 = 3 dd; index2 = 2022 yyyy
		//							the month index starts at 0 so Jan is 0 and Feb is 1 to be used as the selectDateFromDatePicker parameter

		
		//Split the desiredDateTime to get only the Date part
		String date_MM_dd_yyyy[] = (desiredDateTime.split(" ")[0].split("/"));  //we split the entire desiredDateTime at each space (" ") 
																				//and the []=array for index0 split at / and will be 5,3,2022
		
		
		//split time
		//String date_MM_dd_yyyy[] = (desiredDateTime.split("")[1].split(":"));  //[]=array will be 4,40
		...
		int yearDiff = Integer.parseInt(date_MM_dd_yyyy[2]) - Calendar.getInstance().get(Calendar.YEAR); //is the desired year = current year
		...
		//ol = ordered list ; ul = unordered list e.g. ul#datetimepicker_timeview > li:nth-of-type(1)
		//li = list items
		...
		// for (type variableName:  arrayName == type = WebElement; variableName = webElement; arrayName = list_AllTimes
